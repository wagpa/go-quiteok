...
func getInput() (*parameters, error) {
	...
	if code != "encode" && code != "decode" {
		...
		return nil, fmt.Errorf("supported actions are `encode` and `decode`, not %s", code)
	}
	...
	if params.decode {
		if filepath.Ext(params.inPath) != ".qoi" || filepath.Ext(params.outPath) != ".png" {
			return nil, errors.New("`decode` expects the first param to be a qoi and the second a png file")
		}
	} else if filepath.Ext(params.outPath) != ".qoi" || filepath.Ext(params.inPath) != ".png" {
		return nil, fmt.Errorf("`encode` expects the first param to be a png and the second a qoi file, not %s %s", filepath.Ext(params.inPath), filepath.Ext(params.outPath))
	}
